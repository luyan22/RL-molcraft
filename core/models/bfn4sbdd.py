...
class BFN4SBDDScoreModel(BFNBase):
    def __init__(
        ...
        if self.time_emb_dim > 0:
            self.time_emb_layer = TimeEmbedLayer(
                self.time_emb_mode, self.time_emb_dim
            )  # simple mode in default config
        ...
        self.beta1 = torch.tensor(
            beta1, dtype=torch.float32
        )  # type beta, a schedule for types.
    ...
    def interdependency_modeling(
        ...
        theta_h_t,  # noisy ligand_v
        mu_pos_t,  # noisy ligand_pos
    ...
    def loss_one_step(
        ...
        protein_v = [N_protein, 27]  27 = 6 + 20 + 1
        ligand_v = [N_ligand,]   values in int([0, 12])
        """
        K = self.num_classes
        ...
        coord_pred, p0_h, k_hat = self.interdependency_modeling(
            ...
            protein_v=protein_v,
            batch_protein=batch_protein,
            theta_h_t=theta,  # noisy version of ligand_v
            mu_pos_t=mu_coord,  # noisy version of ligand_pos
            batch_ligand=batch_ligand,
    ...
    def sample(
        ...
        for i in trange(1, sample_steps + 1, desc=f"{desc}"):
            ...
            if self.sampling_strategy == "vanilla":
                ...
                t = (
                    torch.ones((n_nodes, 1)).to(self.device) * i / sample_steps
                )  # next time step
                t = t[batch_ligand]
                if self.sampling_strategy == "end_back":
                    theta_h_t = self.discrete_var_bayesian_update(
                        t, beta1=self.beta1, x=sample_pred, K=K
                    )
                elif self.sampling_strategy == "end_back_pmf":
                    theta_h_t = self.discrete_var_bayesian_update(
                        t, beta1=self.beta1, x=p0_h_pred, K=K
                    )
                    ...
                    theta_h_t = self.discrete_var_bayesian_update(
                        t, beta1=self.beta1, x=mode_pred, K=K
                    )
                else:
                    raise NotImplementedError(
                ...
                mu_pos_t = self.continuous_var_bayesian_update(
                    t, sigma1=self.sigma1_coord, x=coord_pred
                )[0]
                sample_traj.append((coord_pred, sample_pred, k_hat))
            ...
            m_hat_traj.append(
                {
                    ...
                    "output": (coord_pred, p0_h_pred, k_hat),
                }
            )
