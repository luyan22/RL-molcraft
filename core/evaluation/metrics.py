...
class CondMolGenMetric(object):
    ...
    def compute_stability(self, generated):  # generated: : list[dict]
    ...
    def compute_chem_results(self, generated):  # generated: list[dict]
        # chem_list = []
        pc = PoseCheck()
                ...
                if self.docking_config is not None:
                    if self.docking_config.mode == "qvina":
                        raise NotImplementedError(
                            "QVina is not supported in this version."
                        )
                        ...
                        vina_results = {"qvina": vina_task.run_sync()[0]["affinity"]}
                        ...
                        score_only_results = vina_task.run(
                            mode="score_only",
                            exhaustiveness=self.docking_config.exhaustiveness,
                        )
                        ...
                        raise NotImplementedError(
                            f"Unknown docking mode: {self.docking_config.mode}"
                        )
    ...
    def evaluate(self, generated, bad_case_dir: str = None):  # generated: list[dict]
        ...
        if "vina" in results[0]:  # TODO try to give vina_score for each ligand
